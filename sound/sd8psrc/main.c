...
)
{
	...
	if (res == FR_OK) {
		...
		do {	/* Data transfer loop */
			...
			if (rb != 1024) {
#ifdef NOAUTOPLAY
				res = FR_OK;	/* File finished, but don't continue with next file */
#endif
...
#ifdef NOAUTOPLAY
			res = 128;	/* Button pressed, continue with next file */
#endif
		} while ((PINB & 1) || ++sw != 1);
	}
	...
	return res;	/* File finished, continue with next file */
}
	...
	for (;;) {
		if (pf_mount(&Fs) == FR_OK) {	/* Initialize FS */
			...
			while (res == FR_OK) {				/* Repeat in the dir */
				...
				while (res == FR_OK || res == 128) {				/* Play all wav files in the dir */
					wdt_reset();
					while (res == FR_OK && (PINB & 1)) wdt_reset();
					...
					if (!(Fno.fattrib & (AM_DIR|AM_HID)) && strstr(Fno.fname, ".WAV")) {
						...
						if (res == FR_OK)	/* File finished, no button pressed */
							res = pf_readdir(&Dir, 0);			/* Rewind dir */
					}
				}
#else
